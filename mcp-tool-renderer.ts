...
export function summarizeProxyMcpCall(args: unknown): string {
  const a = isRecord(args) ? args : {};
  if (typeof a.tool === "string" && a.tool) return `tool ${a.tool}`;
  if (typeof a.connect === "string" && a.connect) return `connect ${a.connect}`;
  if (typeof a.describe === "string" && a.describe) return `describe ${a.describe}`;
  if (typeof a.search === "string" && a.search) return `search ${JSON.stringify(a.search)}`;
  if (typeof a.server === "string" && a.server) return `server ${a.server}`;
  if (typeof a.action === "string" && a.action) return a.action;
  return "status";
}
