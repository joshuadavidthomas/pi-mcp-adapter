...
describe("mcp tool renderer helpers", () => {
  it("summarizes proxy mcp calls by mode", () => {
    expect(summarizeProxyMcpCall({ tool: "django_shell_execute" })).toBe("tool django_shell_execute");
    expect(summarizeProxyMcpCall({ server: "django" })).toBe("server django");
    expect(summarizeProxyMcpCall({ connect: "django" })).toBe("connect django");
    expect(summarizeProxyMcpCall({ search: "models" })).toBe('search "models"');
    expect(summarizeProxyMcpCall({})).toBe("status");
  });
